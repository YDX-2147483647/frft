...
try:
    from pyfftw.interfaces.numpy_fft import fftn, fftshift, ifftn
except:
...
chirp_arg = 1.0j * np.pi * ftools.reduce(lambda x, y: x + y, chirp)
...
def frft_base(arr, alpha):
    phi = alpha * np.pi / 2.0
    cotphi = 1.0 / np.tan(phi)
    cscphi = np.sqrt(1.0 + cotphi**2)
    scale = np.sqrt(1.0 - 1.0j * cotphi) / np.sqrt(np.prod(arr.shape))
...
def RecalculateChirp(newshape):
    ...
    if len(newshape) == 1:  # extra-annoying string manipulations needed with 1D data
        ...
        pref = pref0
        suff = suff0
    regrid = ",".join(tuple("-%d:%d" % (n // 2, n // 2) for n in newshape)).join(
        [pref, suff]
    )
    # print( regrid )
    exec(regrid, globals())
